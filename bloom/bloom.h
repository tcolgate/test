...
bitset bitset_new(int size){
  return (bitset) calloc(size/8,sizeof(char));
};

void bitset_set(bitset bs, uint n){
  int B = n / 8;
  int b = n % 8;
  bs[B] = bs[B] | (1 << b);
};

bool bitset_isset(bitset bs, uint n){
  int B = n / 8;
  int b = n % 8;
  return ((bs[B] & (1 << b)) == (1<<b));
};
...
bloom_t bloom_new(int n, double fprate){
  ...
  b->m = (uint) ceil(((double)n) / log10(fillRatio) *
                  log10(1-fillRatio) / fabs(log(fprate)));
  b->k = (uint) ceil(log2(1/fprate));
